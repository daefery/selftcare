...
CSRContent.controller("UpdateUserDetailController", function ($scope, $route, $http, $window, $location, ChangeUserPropertiesService, GetUserPropertiesService, CSRCache, Notification) {
            ...
            if (updateInfoData.address != notvalid && updateInfoData.phonenumber != notvalid) {
                ChangeUserPropertiesService.update(updateInfoData, function (result) {
                    if (result.$status = 200) {
                        ...
                        setTimeout(function () {
                            window.location.assign('/CSR/Customer/App/MyAccount');
                        }, 3000);
                        //return true;
                    }
                    else {
...
CSRContent.controller("UpdatePasswordUserController", function ($scope, $http, $window, $location, ChangeUserPasswordService, Notification) {
    ...
    var notvalid = "";
        $scope.resetForm = function () {
            ...
            $scope.credentialform.retypenwpassword.$error.passwordVerify = false;
            $scope.credentialform.retypenwpassword.$error.required = true;
            $scope.credentialform.newpassword.$error.required = true;
            $scope.credentialform.oldpassword.$error.required = true;
        };
        $scope.saveChangesPassword = function (data) {
            ...
            if (formData.OldPassword != notvalid && formData.NewPassword != notvalid && formData.OldPassword != null && formData.NewPassword != null) {
...
CSRContent.controller("OverviewController", function ($scope, $filter, $http, ApiConnection, breadcrumbs, CacheMyAccount, CSRCache, GetUserPropertiesService, GetUserDeptbyIdService, Notification) {
