...
CSRContent.controller("CustomerDetailInformation", function ($scope, $rootScope, $http, $q, ApiConnection, breadcrumbs, SearchCustomerDetailService, CacheSearch, CSRCache, CommonEnum, pinnedHtmlService, $filter, $parse, CSRGetCustomerBalanceService, GetOrdersbyFilterService, ErrorHandlerUtility, ValidateUser, LocalStorageProvider) {
    ...
    $scope.load_CustomerDetail = function () {
        if (!customerId) {
        ...
        CacheSearch.getSearchDetailCustomer().then(function (result) {
            ...
            var pinned = { pinnedId: result.CustomerID, pinnedName: result.CustomerData.FirstName + ' ' + result.CustomerData.LastName, pinnedPhone: result.CustomerData.MobileNumber };
            ...
            $scope.resetEditDetailForm = function () {
                ...
                if ($scope.update.CustomerData.BirthDay != null) {
                    $scope.update.CustomerData.BirthDay = moment($scope.update.CustomerData.BirthDay).format(config.DateFormatMoment);
                }
            }
            ...
            if ($scope.products != null) {
                ...
                $rootScope.nationality = CommonEnum.convertCountryList($scope.products.CustomerData.Nationality).name;
                ...
                var nationality = $parse("update.CustomerData.Nationality");
                var birthday = null;
                if ($scope.update.CustomerData.BirthDay != null) {
                    birthday = moment($scope.update.CustomerData.BirthDay).format(config.DateFormatMoment);
                }
                ...
                nationality.assign($scope, CommonEnum.convertCountryList($scope.update.CustomerData.Nationality));
    ...
    $scope.load_CustomerDetail();

    $scope.$on('refresh-customerDetail', function (event, args) {
...
CSRContent.controller('CustDetailModalController', function ($scope, $modal, $parse, CSRCache, CommonEnum, $filter) {
    ...
    $scope.openEditStatus = function (size) {
        ...
        angular.element('#ModifyCustomerStatusModal').modal('show');
        $scope.ModifyStatus = { name: 'Active', value: 'Active' };
        if ($scope.products.customerStatus != undefined) {
            $scope.ModifyStatus = {
                name: $scope.products.customerStatus,
                value: $scope.products.customerStatus
            };
        }
    ...
    $scope.ModifyLanguageName = {};
    $scope.openEditLanguage = function (size) {

        angular.element('#ModifyCustomerLanguageSettingModal').modal('show');
        $scope.ModifyLanguageName = { name: 'English', value: 'English' };
        if ($scope.products.CustomerData != undefined) {
            $scope.ModifyStatus = {
                name: $scope.products.CustomerData.LanguageName,
                value: $scope.products.CustomerData.LanguageName
            };
        }
