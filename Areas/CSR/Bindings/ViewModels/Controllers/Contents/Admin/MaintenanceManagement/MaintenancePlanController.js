CSRContent.controller("MaintenancePlanController", function ($scope, $filter, GetAllMaintenancePlan, RemoveMaintenancePlan, Notification, CacheAdmin) {
    ...
    $scope.refresheditform = function (item) {
        ...
        $scope.editformold = angular.copy($scope.editform);
        $scope.mvnoselectionold = angular.copy($scope.mvnoselection);
    }
...
CSRContent.controller('AddMaintenanceForm', function ($scope, $parse, CreateMaintenancePlan, Notification) {
    ...
    $scope.validateURL = function () {
        if ($scope.addmaintenanceform.url.$invalid) {
            //show notif error
            Notification.error({
    ...
    $scope.setPristineAddForm = function () {
        $scope.addmaintenanceform.$setPristine();
...
CSRContent.controller('EditMaintenanceForm', function ($scope, $parse, CreateMaintenancePlan, UpdateMaintenancePlan, Notification) {
    ...
    $scope.validateURL = function () {
        if ($scope.Editmaintenanceform.url.$invalid) {
            //show notif error
            Notification.error({
                message: '<span>' + "URL should have prefix http:// or https://" + '</span>',
                positionY: 'top',
    ...
    $scope.setPristineEditForm = function () {
        $scope.Editmaintenanceform.$setPristine();
    }

    $scope.isDirty_Editmaintenanceform = function () {
        var dirty = false;
        ...
        if ($scope.editform.MaintenanceDate != $scope.editformold.MaintenanceDate) dirty = true;
        else if ($scope.editform.timestart.getTime() != $scope.editformold.timestart.getTime()) dirty = true;
        else if ($scope.editform.MaintenanceDuration != $scope.editformold.MaintenanceDuration) dirty = true;
        else if ($scope.editform.Description != $scope.editformold.Description) dirty = true;
        else if ($scope.editform.Detail != $scope.editformold.Detail) dirty = true;
        else if ($scope.editform.URLReference != $scope.editformold.URLReference) dirty = true;
        else if (isSelectionDifferent($scope.mvnoselection, $scope.mvnoselectionold)) dirty = true;
        return dirty;
    }
