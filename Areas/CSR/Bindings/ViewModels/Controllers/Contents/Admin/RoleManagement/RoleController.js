...
CSRContent.controller('viewRoleCtrl', function ($scope, $rootScope, $route, $http, $window, $location, GetRolebyMVNOIdService, RoleManagementUtility, GetRoleDetailbyIdService, GetUserbyRoleIdService, GetRoleModuleTreebyId, DeleteRolebyIdService, Notification, GetModuleinTreebyParentIDService, CSRCache, CreateRoleService, GetRoleIDbyNameService, CreateRoleModuleTreeService, UpdateRoleService, ReplaceRoleModuleService, CacheAdmin, GetRoleModuleListbyRoleIdService, GetSSbyRoleService, GetAllSecureSectionService, CreateRoleSectionbyListService, GetModuleTreebyMVNOAccessService, ModuleIdbyRoleService, LocalStorageProvider, GetSecureSectionbyRoleDomainService, CSRSecureSection, GetSecureSectionbyRoleDomainwithBracketService) {

    var $myGroup = $('#myGroup');
    $scope.selectedRow = 0;
    $scope.hideclick = function (vari) {
        $scope.selectedRow = vari;
        lastclicked = vari;
        $("#22").removeClass(".in");
        //if (lastclicked != null) {
        //    var temp = String(lastclicked);
        //    $(temp).removeClass("in");
        //}
    };

    $scope.showaddrolemodal = false;
    $scope.showmodrolemodal = false;
    $scope.showssmodal = false;

    $scope.showmodal = function (adjtype) {
        if (adjtype == 1) {
            $scope.showaddrolemodal = true;
            $scope.showmodrolemodal = false;
            $scope.showssmodal = false;
        }
        else if (adjtype == 2) {
            $scope.showaddrolemodal = false;
            $scope.showmodrolemodal = true;
            $scope.showssmodal = false;
        }
        else if (adjtype == 3) {
            $scope.showaddrolemodal = false;
            $scope.showmodrolemodal = false;
            $scope.showssmodal = true;
        }
    ...
    $scope.isRoleChanged = function () {
        ...
        var temp2 = [];
        if (oldlist != null) { temp2 = angular.copy(oldlist); }
        var result = 0;
        var containall = true;
        ...
        if ($scope.products != null) {
            $scope.temporal = angular.copy($scope.products);
        }
        $scope.tempmodaldata = $scope.temporal;
        if ($scope.tempmodaldata.IsActive != null) {
            if ($scope.tempmodaldata.IsActive == "1") {
                $scope.tempmodaldata.IsActive = true
            }
            else {
                $scope.tempmodaldata.IsActive = false
            }
        }
        if (angular.equals($scope.tempmodaldata, $scope.oldrolename) == true) {
            ...
            //    return false;
            //}
            if (result == temp2.length && containall == true) {
                return true;
            }
            else {
                return false;
    ...
    $scope.isRoleSSChanged = function () {
        var temp = [];
        if ($scope.roleSSlist != null) {
            temp = angular.copy($scope.roleSSlist);
        }
        var temp2 = [];
        if ($scope.oldroleSSlist != null) { temp2 = angular.copy($scope.oldroleSSlist); }
        var result = 0;
        var containall = true;
        for (var index = 0; index < temp.length; index++) {
            var item = temp[index];
    ...
    resetValue = function () {
        ...
        $scope.moduletree = {};
        $scope.roleusers = [];
        var temp = RoleManagementUtility.GetEmail();
        var parent = $scope.moduleparent;
        GetRolebyMVNOIdService.query({ MvnoId: temp, TypeId: $scope.roletypeid }, function (result) {
    ...
    $scope.emptylistthenfill = function () {
        ...
        //$scope.oldrolelist = CSRCache.get('moduleidlistbyrole');
    };
    ...
    $scope.getroleid = function (id) {
        ...
        GetRoleDetailbyIdService.query({ RoleID: id }, function (result) {
            ...
            $scope.oldrolename = angular.copy(result);
        ...
        GetSSbyRoleService.query({ RoleId: id }, function (result) {
            ...
            if (data.length > 0) {
                for (var index = 0; index < data.length; index++) {
                    ...
                    $scope.oldroleSSlist.push(item.SectionID);
                };
            }
        })
        ...
        ModuleIdbyRoleService.query({ RoleId: id, MvnoId: mvno }, function (result) {
            ...
            //CSRCache.put('oldmoduleidlistbyrole', result);
            oldlist = angular.copy(result);
        })
        GetSecureSectionbyRoleDomainwithBracketService.query({ domainId: $scope.moduleparent, roleId: id }, function (result) {
            $scope.allsecsec = angular.copy(result);
        })
